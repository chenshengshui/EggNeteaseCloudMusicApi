...
const aesEncrypt = (buffer, mode, key, iv) => {
  const cipher = cryptoJs.createCipheriv('aes-128-' + mode, key, iv);
...
const rsaEncrypt = (buffer, key) => {
  ...
  return cryptoJs.publicEncrypt(
    { key, padding: cryptoJs.constants.RSA_NO_PADDING },
    buffer
  );
};
...
const eapi = (url, object) => {
  ...
  const digest = cryptoJs.createHash('md5').update(message).digest('hex');
...
const decrypt = (cipherBuffer: any) => {
  const decipher = cryptoJs.createDecipheriv('aes-128-ecb', eapiKey, '');
